...
DEST = ROOT / "dist/NoMansSkyScanner-v1.0.0-NMS170671"
...
def main():
    ...
    excluded = {"site-packages", "idlelib", "tkinter", "turtledemo", "test", "tests",
                "ensurepip", "venv", "__pycache__"}
    library = RUNTIME / "Lib"
    ...
    copytree(ROOT / "nms_scanner", DEST / "nms_scanner",
             shutil.ignore_patterns("*.pyc", "__pycache__"))
    copytree(ROOT / "native", DEST / "native", shutil.ignore_patterns("*.pyc", "__pycache__"))
    (DEST / "tools").mkdir()
    shutil.copy2(ROOT / "tools/audit_native.py", DEST / "tools/audit_native.py")
    for name in ("README.md", "requirements.lock.txt", "AGENTS.md", "PROJECT_CONTEXT.md"):
        shutil.copy2(ROOT / name, DEST / name)
    (DEST / "research").mkdir()
    for name in (
        "UPLOAD_FINDINGS.md", "automatic-trial-20260903T1049.json",
        ...
        json.dumps(
            {
                ...
                "hotkeys": {"F1": "start_pause_resume", "F2": "toggle_auto_upload",
                            "F3": "stop"},
    ...
    launcher_source.write_text(
        ...
        ' if(!File.Exists(py)){\n'
        '  Console.Error.WriteLine("发行包不完整：找不到 runtime\\\\python.exe");return 2;}\n'
        ' string q="-u -X utf8 -m nms_scanner.launcher --loop";\n'
    ...
    subprocess.run(
        [str(compiler), "/nologo", "/optimize+", "/target:exe",
         "/out:" + str(DEST / "NoMansSkyScanner.exe"), str(launcher_source)], check=True
    )
    (DEST / "启动自动探索.bat").write_text(
        '@echo off\r\ncd /d "%~dp0"\r\nNoMansSkyScanner.exe\r\npause\r\n', encoding="utf-8-sig"
    )
