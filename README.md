...
这是加入动物、植物、矿物扫描之前的 **v1.1.0 图形界面版本**。整系扫描登记行星发现，不会自动发现星球上的动物、植物或矿物。

[下载 Windows 便携版](https://github.com/yuhan2680/NoMansSkyScanner/releases/tag/v1.1.0)。下载 `NoMansSkyScanner-v1.1.0-NMS170671.zip` 后完整解压；不要只移动 EXE，运行时和配置目录必须一起保留。附带 SHA-256 校验文件。
...
- 68 项非游戏测试、Ruff、编译检查和当前 EXE 的 16 个原生入口核查通过。UI 的命令鉴权、顺序去重和状态投影已自动测试；界面按钮还需要一次游戏内验收。
